...
tools: Read, Write, Edit, mcp__akshare__*
---
...
3. **Pull data.** Use the AkShare MCP for A-share trading multiples, financial statements, and industry data. Use `get_industry_stocks(industry)` to build the peer set.
...
- **Cite every number.** If a multiple can't be sourced from AkShare, flag it as `[UNSOURCED]`.
