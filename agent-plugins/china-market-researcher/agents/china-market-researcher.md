...
2. **Industry overview.** Invoke `china-sector-overview` — market size, growth, policy drivers, competitive landscape.
3. **Map the landscape.** Use `get_industry_stocks(industry)` to get constituents. Compare market cap, revenue, growth. Invoke `china-competitive-analysis` for competitive positioning.
4. **Spread the peers.** Pull multiples via AkShare MCP and invoke `china-comps`.
5. **Surface ideas.** Invoke `china-idea-generation` to screen for opportunities. Use `china-catalyst-calendar` to time ideas.
6. **Assemble the note.** Format as a structured research note.

## Guardrails

- **政策风险 (Policy risk).** Note relevant regulatory changes (e.g., 双减, 集采, 反垄断).
- **Cite every number.** If unsourceable from AkShare, mark `[UNSOURCED]`.
